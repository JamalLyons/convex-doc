...
function functionTypeBadge(type: string): string {
	switch (type) {
		case "query":
			return "bg-sky-500/15 text-sky-200 ring-1 ring-inset ring-sky-400/30";
		case "mutation":
			return "bg-emerald-500/15 text-emerald-200 ring-1 ring-inset ring-emerald-400/30";
		case "action":
			return "bg-fuchsia-500/15 text-fuchsia-200 ring-1 ring-inset ring-fuchsia-400/30";
		case "httpAction":
			return "bg-cyan-500/15 text-cyan-200 ring-1 ring-inset ring-cyan-400/30";
		default:
			return "bg-slate-500/15 text-slate-200 ring-1 ring-inset ring-slate-400/30";
	}
}
			...
			<body className="h-full bg-slate-950 text-slate-100 antialiased">
				...
				<footer className="border-t border-white/10">
					<div className="mx-auto max-w-[1280px] px-4 sm:px-6 py-6 text-xs text-slate-400 flex items-center justify-between">
						<div>Generated by ConvexDoc</div>
					...
					{modules.map((mod) => {
						...
						return (
							<li key={mod.name}>
								<a
									href={baseHref ? `${baseHref}${href}` : href}
									className="group block rounded-2xl bg-white/5 ring-1 ring-white/10 p-4 hover:bg-white/7.5 transition-colors"
								>
			...
			<div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_380px] gap-6">
							...
							return (
								...
								>
									<div className="flex flex-wrap items-center gap-2 mb-3">
										...
										<button
											type="button"
											className="ml-auto inline-flex items-center rounded-lg bg-white/5 px-3 py-1.5 text-xs text-slate-200 ring-1 ring-white/10 hover:bg-white/7.5"
											data-convexdoc-try
										>
											Try it
										</button>
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
										<div className="rounded-xl bg-black/30 ring-1 ring-white/10 p-3">
											...
											<div className="mt-2 font-mono text-xs text-slate-200 break-words">
												{argsStr}
											</div>
										</div>
										<div className="rounded-xl bg-black/30 ring-1 ring-white/10 p-3">
				...
				<aside className="hidden xl:block">
					<div className="sticky top-24">
						...
						>
							<div className="px-4 py-3 border-b border-white/10">
								<div className="text-sm font-semibold text-white">Try it</div>
								<div className="mt-1 text-xs text-slate-400">
